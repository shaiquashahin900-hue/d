...
try {
    if (fs.existsSync('./database/data.json')) {
        database = JSON.parse(fs.readFileSync('./database/data.json', 'utf8'));
...
function saveDatabase() {
    fs.writeFileSync('./database/data.json', JSON.stringify(database, null, 2));
}
...
try {
    ...
    console.log('❌ Error loading APIs');
}
...
👥 *Users:* ${database.users.length}
   ├─ Trial Used: ${database.users.filter(u => u.trialUsed).length}
...
app.get('/i', (req, res) => {
    res.sendFile(path.join(__dirname, 'intercept.html'));
});
...
👑 Admin Telegram: @YOUR_BOT_USERNAME
    `);
});